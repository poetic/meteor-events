Meteor.startup(function(){
  ...
  var auser2, auser3, auser4, auser5, auser6;

  if (Events.find().count()) {
    auser2 = Meteor.users.findOne({ _id: user2 })
    auser3 = Meteor.users.findOne({ _id: user3 })
    auser4 = Meteor.users.findOne({ _id: user4 })
    auser5 = Meteor.users.findOne({ _id: user5 })
    auser6 = Meteor.users.findOne({ _id: user6 })
  }
  ...
  if (Meteor.users.find().count() && Events.find().count()) {
    Meteor.users.update({ _id: user2 }, { $push: { 'profile.events': { $each: [events[0]] } } })
    Meteor.users.update({ _id: user3 }, { $push: { 'profile.events': { $each: [events[0]] } } })
    Meteor.users.update({ _id: user4 }, { $push: { 'profile.events': { $each: [events[0], events[1]] } } })
    Meteor.users.update({ _id: user5 }, { $push: { 'profile.events': { $each: [events[0], events[1]] } } })
  }
});
